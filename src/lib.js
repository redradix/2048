...
const collapseRows = (direction, board) => {
  return board.map(
    direction === 'left' ? collapseRowToLeft : collapseRowToRight
  )
}
...
const collapseColumns = (direction, board) => {
  ...
  return transposed.map(
    direction === 'up' ? collapseRowToLeft : collapseRowToRight
  )
}

module.exports = (direction, board) =>
  rowDirections.includes(direction)
    ? collapseRows(direction, board)
    : collapseColumns(direction, board)
